...
function switchPanel(name) {
  ...
  const titles = {
    ...
    products:'제품 관리', science:'과학/기술', academy:'아카데미', news:'뉴스 관리',
    media:'미디어 라이브러리', json:'JSON 편집기'
  };
...
const MEDIA_KEY = 'aestyve_media_library';
...
function renderMediaGrid(filter) {
  ...
  const countEl = document.getElementById('mediaCountInfo');
  if (!grid) return;

  let list = getMediaList();
  const filterVal = filter || (document.getElementById('mediaFilter') ? document.getElementById('mediaFilter').value : 'all');

  if (filterVal === 'image') list = list.filter(m => isImage(m.type));
  else if (filterVal === 'video') list = list.filter(m => !isImage(m.type));

  // 기존 카드 제거 (empty 제외)
  grid.querySelectorAll('.media-card').forEach(c => c.remove());

  if (countEl) {
    const total = getMediaList().length;
    const images = getMediaList().filter(m => isImage(m.type)).length;
    const videos = total - images;
    countEl.textContent = `전체 ${total}개 (사진 ${images}개 · 동영상 ${videos}개)`;
  }
  ...
  list.forEach(function(item) {
    ...
    const typeClass = isImage(item.type) ? 'type-image' : 'type-video';
    const typeLabel = isImage(item.type) ? '사진' : '동영상';

    let thumbHtml = '';
    if (isImage(item.type)) {
      thumbHtml = `<img class="media-card-thumb" src="${item.dataUrl}" alt="${item.name}" onclick="openMediaViewer('${item.id}')" style="cursor:pointer;">`;
    } else {
      thumbHtml = `<div class="media-card-thumb video-card-thumb" onclick="openMediaViewer('${item.id}')" style="cursor:pointer;">🎬</div>`;
    }

    card.innerHTML = `
      ${thumbHtml}
      <div class="media-card-body">
        <span class="media-card-type-badge ${typeClass}">${typeLabel}</span>
        <div class="media-card-name">${item.name}</div>
        <div class="media-card-meta">${formatFileSize(item.size)} · ${item.date}</div>
        <div class="media-card-actions">
          <button class="btn-copy-url" onclick="copyMediaUrl('${item.id}', this)">🔗 URL 복사</button>
          <button class="btn-media-delete" onclick="deleteMedia('${item.id}')">🗑</button>
        </div>
      </div>`;
...
function copyMediaUrl(id, btn) {
  ...
  navigator.clipboard.writeText(item.dataUrl).then(function() {
    ...
    // clipboard API 실패 시 대체
    const ta = document.createElement('textarea');
    ta.value = item.dataUrl;
...
function clearAllMedia() {
  if (!confirm('라이브러리의 모든 파일을 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.')) return;
  localStorage.removeItem(MEDIA_KEY);
  renderMediaGrid();
  showToast('전체 파일이 삭제되었습니다.', 'success');
}
...
function openMediaViewer(id) {
  ...
  let viewer = document.getElementById('mediaViewerOverlay');
  if (!viewer) {
    viewer = document.createElement('div');
    viewer.className = 'media-viewer-overlay';
    viewer.id = 'mediaViewerOverlay';
    viewer.innerHTML = `
      <div class="media-viewer-inner" id="mediaViewerInner">
...
// 파일 처리 (FileReader로 Base64/ObjectURL 변환)
function processFiles(files) {
  ...
  Array.from(files).forEach(function(file) {
    ...
    if (file.size > 100 * 1024 * 1024) {
      showToast(`${file.name}: 파일 크기가 100MB를 초과합니다.`, 'error');
      return;
    }

    const itemId = 'up_' + Date.now() + '_' + Math.random().toString(36).slice(2, 7);

    // 진행 아이템 생성
    const progressItem = document.createElement('div');
    progressItem.className = 'upload-progress-item';
    progressItem.id = 'progress_' + itemId;

    const thumbHtml = isImage(file.type)
      ? `<img class="upload-progress-thumb" id="thumb_${itemId}" src="" alt="">`
      : `<div class="upload-progress-thumb video-thumb">🎬</div>`;

    progressItem.innerHTML = `
      ${thumbHtml}
      <div class="upload-progress-body">
        <div class="upload-progress-name">${file.name}</div>
        <div class="upload-progress-size">${formatFileSize(file.size)}</div>
        <div class="upload-progress-bar-wrap">
          <div class="upload-progress-bar" id="bar_${itemId}" style="width:0%"></div>
        </div>
      </div>
      <span class="upload-progress-status uploading" id="status_${itemId}">읽는 중...</span>`;
    ...
    reader.onload = function(e) {
      ...
      if (isImage(file.type)) {
        const thumb = document.getElementById('thumb_' + itemId);
      ...
      const status = document.getElementById('status_' + itemId);
      if (status) { status.textContent = '완료'; status.className = 'upload-progress-status done'; }
      ...
      // 3초 후 진행 아이템 제거
      setTimeout(function() {
        const el = document.getElementById('progress_' + itemId);
        if (el) el.remove();
      }, 3000);
    };
    ...
    reader.onerror = function() {
      const status = document.getElementById('status_' + itemId);
      if (status) { status.textContent = '오류'; status.className = 'upload-progress-status error'; }
      showToast(`${file.name} 업로드 중 오류가 발생했습니다.`, 'error');
    };
